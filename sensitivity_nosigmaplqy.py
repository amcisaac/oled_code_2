...
bounds = [[rates[0]*lb,rates[0]*ub],[rates[1]*lb,rates[1]*ub],[rates[2]*lb,rates[2]*ub],
          [rates[3]*lb,rates[3]*ub],[rates[5]*lb,rates[5]*ub],[rates[7]*lb,rates[7]*ub]]
#no 4 (sigma), 6(plqy)
Vol = Vol**3
...
print("Considering effect of rec, kr, keea, keca, kbi, brec only. Bounds: {}k to {}k".format(lb,ub))
...
def obj_2(param_value,rates,jobtyp,fit,x_min,x_max,disr,Vol,shift):
    ...
    all_params[0] = param_value[0]
    all_params[1] = param_value[1]
    all_params[2] = param_value[2]
    all_params[3] = param_value[3]
    all_params[4] = rates[4]
    all_params[5] = param_value[4]
    all_params[6] = rates[6]
    all_params[7] = param_value[5]
...
tot_var = np.var(Y)
mean_var = np.mean(Y)
print('Mean value of lsq in the system: ', mean_var)
print('Total variance in the system: ',tot_var)
